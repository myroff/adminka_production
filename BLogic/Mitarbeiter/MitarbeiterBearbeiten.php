...
class MitarbeiterBearbeiten
{
	public function showList()
	{
		...
		include_once BASIS_DIR.'/Templates/Mitarbeiter/MitarbeiterBearbeitenListe.tmpl.php';
		return;
	}
	...
	public function showMitarbeiterById($mtId)
	{
		require_once BASIS_DIR.'/MVC/DBFactory.php';
		$dbh = \MVC\DBFactory::getDBH();
		if(!$dbh)
		...
		if(isset($itemName) AND isset($itemValue))
		{
			...
			$meldung .= "<br>" . self::updateItemInDB($mtId,$itemName,$itemValue);
		}
		...
		include_once BASIS_DIR .'/Templates/Mitarbeiter/MitarbeiterBearbeitenById.tmpl.php';
		return;
	}
	
	private function updateItemInDB($mId, $itemName, $itemVal)
	{
		...
		$q = "UPDATE mitarbeiter SET $in = :itemVal WHERE mtId = $mId";
