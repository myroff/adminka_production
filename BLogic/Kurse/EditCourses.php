...
class EditCourses
{
    public function showList()
    {
        ...
        include_once BASIS_DIR.'/Templates/Kurse/EditCoursesList.tmpl.php';
        return;
    }
    ...
    public function showCourseById($kurId)
    {
        $meldung;
        ...
        try
        {
            ...
            #$sth->execute(array(":kurId" => $kurId));
            #$seasonsConfigs = $sth->fetchAll(PDO::FETCH_ASSOC);
            $courseModel = new \Kurse\CourseToSeasonsModel();
            $seasonsConfigs = $courseModel->getFrontendInfoToCourseId($kurId);
        ...
        switch ($itemName)
        {
            ...
            case "isKurInactive":
                if($itemVal === "ja" OR $itemVal === "nein")
                {
                    ...
                    $dataPost[':isKurInactive'] = ($itemVal === "ja") ? 0 : 1;
                }
                else
                {
