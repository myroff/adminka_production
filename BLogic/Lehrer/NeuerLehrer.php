...
class NeuerLehrer
{
	...
	public function saveNewLehrer()
	{
		...
		if(!empty($_POST['fach']))
		{
			$_POST['fach'] = trim($_POST['fach']);
			$_POST['fach'] = str_replace("/\s+/", " ", $_POST['fach']);
			if(Fltr::isText($_POST['fach']))
			{
				$dataPost[':fach'] = $_POST['fach'];
			}
			else
			{
				$fehler .= "Fach(-er) ist(sind) falsch eingegeben.<br>";
				$fehler .= "Erlaubt sind Ziffern 0 bis 9, Buchstaben, Leerzeichen, Symbolen (. , : ; ! ? ' * = # / \ \" - + _).<br>";
				$fehlerInput[] = 'fach';
			}
		}
		...
		if(empty($fehler))
		{
			...
			try
			{
				...
				else
				{
					...
					if($res>0)
					{
						$output = array('info' => "[DB] Neuer Lehrer wurde erfolgreich hinzugefügt.", 'data' => $dataPost);
					}
					else
					{
						$output = array('info' => "[DB] Neuer Lehrer konnte nicht hinzugefügt werden.", 'data' => $dataPost);
					}
				}
			}
			catch (Exception $ex) {
